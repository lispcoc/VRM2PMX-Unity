        ...
        try
        {
            ...
            {//vertex face blendshape
                ...
                for (int i = 0; i < orgRenderers.Length; i++)
                {
                    ...
                    if (!Directory.Exists(Path.GetDirectoryName(savepath) + Path.DirectorySeparatorChar + "tex"))
                        Directory.CreateDirectory(Path.GetDirectoryName(savepath) + Path.DirectorySeparatorChar + "tex");

                    if (createExTexture)
                    {
                        foreach (var material in renderer.sharedMaterials)
                        {
                            string filePath = Path.GetDirectoryName(savepath) + Path.DirectorySeparatorChar + "tex" + Path.DirectorySeparatorChar + material.name + ".png";
                    ...
                    mattex.AddRange(renderer.sharedMaterials.Select(x =>
                    {
                        ...
                        */
                        ...
                        {
                            string filePath = Path.GetDirectoryName(savepath) + Path.DirectorySeparatorChar + "tex" + Path.DirectorySeparatorChar + mainTex.name + ".png";
                ...
                {
                    int textureIndex = ArrayUtility.IndexOf(pmx.TextureFiles, x.texture.Key);
                    if (textureIndex < 0)
                        textureIndex = ArrayUtility.IndexOf(pmx.TextureFiles, x.texture.Value);
            ...
            #region Rigid          //Rigid
            {
                ...
                foreach (var sBone in springBones)
                {
                    ...
                    ushort colider = (ushort)(sBone.ColliderGroups != null ? (0 < sBone.ColliderGroups.Count() ? 0xFFFE : 0) : 0);
...

            {
                ...
                if (metaContainer != null)
                {
                    ...
                    descBuilder.Append("\n");
                    if (0 < meta.OtherLicenseUrl.Length)
                    {
            ...
            if (createExTexture)
            {
                ...
                File.Copy("Assets/PMXExporter/mtlike.fx", efPath, true);
